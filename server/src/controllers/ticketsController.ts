...
import { insertTicket, updateTicket as updateTicketStatus, fetchTickets, fetchTicket } from '../services/ticketsServices';
...
export const getTickets = async (req : Request, res : Response) => {
    try {
        ...
        if(data.status == 200){
          res.json({status: data.status, statusText: data.statusText, data: data.data }); 
        }else{
          res.status(data.status).json({ error: data.statusText });
        }
      } catch (err : any) {
...
export const respondToTicket = async (req : Request, res : Response) => {
    ...
    res.json({status: 200, statusText: "Email Sent!" }); 
}
